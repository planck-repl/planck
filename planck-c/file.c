...
uint64_t ufile_open(const char *path, const char *encoding, const char *mode) {
...
uint64_t file_to_descriptor(FILE *file) {
    return (uint64_t) file;
}

FILE *descriptor_to_file(uint64_t descriptor) {
    return (FILE *) descriptor;
}

uint64_t file_open(const char *path, const char *mode) {
...
size_t file_read(uint64_t descriptor, size_t buf_size, uint8_t *buf) {
    FILE *file = descriptor_to_file(descriptor);
    return fread(buf, sizeof(uint8_t), buf_size, file);
}

void file_write(uint64_t descriptor, size_t buf_size, uint8_t *buf) {
    FILE *file = descriptor_to_file(descriptor);
